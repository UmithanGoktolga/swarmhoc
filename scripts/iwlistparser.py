...
# NEED TO BE CHANGED BEFORE RUNNING PUBLISHER (wlan0, "testhoc")
interface = "wlp3s0"
networkName = "FiberHGW_ZTZS66_2.4GHz"
...
def main():
    ...
    global quality_percent
    global signal_level


    for netw in parsed_cells:
        if netw["Name"] == networkName:
            quality_percent = netw["Quality"]
            signal_level = netw["Signal"]
            break

    try: 
        results.append(int(quality_percent.split()[0]))
        results.append(int(signal_level.split()[0]))
    except :
        results.append(quality_percent)
        results.append(signal_level)
